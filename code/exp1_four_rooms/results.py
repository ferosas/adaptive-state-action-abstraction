"""Build report-style CSV tables from saved Four-Rooms experiment results."""
...
DEFAULT_RESULTS_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "four_rooms"


def parse_args() -> argparse.Namespace:
    """Define the CLI for Four-Rooms report-table extraction."""
    parser = argparse.ArgumentParser(
        description="Build report-style CSV tables from saved Four-Rooms results."
    )
...
def _rebuild_four_rooms_fixed_family_table(summary: Dict[str, object]) -> List[Dict[str, object]]:
...
def build_report_csvs(results_dir: Path, output_dir: Path) -> None:
    """Build and save the Four-Rooms report CSVs."""
    ...
    if not table1_rows:
        table1_rows = _rebuild_four_rooms_fixed_family_table(summary)
...
def main() -> None:
    """Entry point for rebuilding Four-Rooms report CSVs."""
    args = parse_args()
    output_dir = (
