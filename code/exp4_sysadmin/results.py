"""Build report-style CSV tables from saved SysAdmin experiment results."""
...
DEFAULT_RESULTS_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "sysadmin"


def parse_args() -> argparse.Namespace:
    """Define the CLI for SysAdmin report-table extraction."""
    parser = argparse.ArgumentParser(
        description="Build report-style CSV tables from saved SysAdmin results."
    )
...
def _rebuild_sysadmin_fixed_family_table(summary: Dict[str, object]) -> List[Dict[str, object]]:
    ...
    distortion = sysadmin_mdp.load_distortion(
        mdp,
        metric_kind=str(config.get("metric_kind", "one_step")),
        verbose=False,
    )
...
def build_report_csvs(results_dir: Path, output_dir: Path) -> None:
    """Build and save the SysAdmin report CSVs."""
    ...
    if not table1_rows:
        table1_rows = _rebuild_sysadmin_fixed_family_table(summary)
...
def main() -> None:
    """Entry point for rebuilding SysAdmin report CSVs."""
    args = parse_args()
    output_dir = (
