"""Build report-style figures from SysAdmin postprocessed CSV tables."""
...
DEFAULT_RESULTS_DIR = (
    Path(__file__).resolve().parent.parent.parent / "results" / "sysadmin" / "final_results"
)
DEFAULT_FIGURES_DIR = (
    Path(__file__).resolve().parent.parent.parent / "results" / "sysadmin" / "figures"
)
FIGURE1_X_MAX = 30.0
FIGURE2_X_MAX = 30.0
...
def parse_args() -> argparse.Namespace:
    """Define the CLI for rebuilding SysAdmin report figures."""
    parser = argparse.ArgumentParser(
        description="Build report-style figures from SysAdmin final_results CSVs."
    )
...
def make_figure4(results_dir: Path, output_dir: Path) -> Path:
    ...
    for label in ordered_labels:
        ...
        if not method_rows:
            continue
        if str(method_rows[0].get("method_type", "")) != "fixed":
            continue
    ...
    limits = AP.compute_visible_y_limits(
        ...
        include_values=[
            ...
            for row in rows
            if row.get("method_type") == "fixed"
            and row.get("abstraction_error") not in (None, "")
        ],
    )
...
def make_all_figures(results_dir: Path, output_dir: Path) -> None:
    """Build every SysAdmin report figure."""
...
def main() -> None:
    """Entry point for rebuilding SysAdmin figures."""
    args = parse_args()
    for legacy_path in [
