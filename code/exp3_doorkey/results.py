"""Build report-style CSV tables from saved DoorKey experiment results."""
...
DEFAULT_RESULTS_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "doorkey"


def parse_args() -> argparse.Namespace:
    """Define the CLI for DoorKey report-table extraction."""
    parser = argparse.ArgumentParser(
        description="Build report-style CSV tables from saved DoorKey results."
    )
...
def _rebuild_doorkey_fixed_family_table(summary: Dict[str, object]) -> List[Dict[str, object]]:
...
def build_figure2_rows(trace_rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    """Build Figure 2 data: policy-return data against normalized compute."""
    rows: List[Dict[str, object]] = []
    for row in trace_rows:
...
def build_figure3_rows(trace_rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    """Build Figure 3 data: policy-return data against raw Bellman updates."""
    rows: List[Dict[str, object]] = []
    for row in trace_rows:
        if row.get("policy_return") in (None, ""):
            continue
...
def build_figure4_rows(trace_rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    """Build Figure 4 data: induced-value-error against Bellman update."""
    rows: List[Dict[str, object]] = []
    for row in trace_rows:
...
def main() -> None:
    """Build and save the DoorKey report CSVs."""
    args = parse_args()
    results_dir = args.results_dir
    output_dir = args.output_dir or (results_dir / "final_results")
    output_dir.mkdir(parents=True, exist_ok=True)

    summary = AC.load_summary(results_dir / "summary.json")
    trace_rows = AC.add_normalized_bellman_compute(
        AC.load_rows(results_dir / "traces.csv"),
        summary,
    )
    config = summary.get("config", {})
    if not isinstance(config, dict):
    ...
    if not table1_rows:
        table1_rows = _rebuild_doorkey_fixed_family_table(summary)
    ...
    figure2_rows = build_figure2_rows(trace_rows)
    figure3_rows = build_figure3_rows(trace_rows)
    figure4_rows = build_figure4_rows(trace_rows)
    figure5_rows = build_figure5_rows(trace_rows)

    AC.save_rows(output_dir / "report_table1.csv", table1_rows)
    AC.save_rows(output_dir / "report_figure0.csv", figure0_rows)
    AC.save_rows(output_dir / "report_figure1.csv", figure1_rows)
    AC.save_rows(output_dir / "report_figure2.csv", figure2_rows)
    AC.save_rows(output_dir / "report_figure3.csv", figure3_rows)
    AC.save_rows(output_dir / "report_figure4.csv", figure4_rows)
    AC.save_rows(output_dir / "report_figure5.csv", figure5_rows)
